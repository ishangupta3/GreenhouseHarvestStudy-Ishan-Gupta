...
class HarvestPlanItem(BaseModel):
    ...
    crop_type: str
    sub_crop: str
    planned_yield_g: float  # Changed from int to float to handle decimal values
...
# New API endpoints for harvest planning
...
async def add_to_harvest_plan(target_date: str, item: HarvestPlanItem):
    ...
    try:
        ...
        # Check if crop is already in plan for this date
        cursor.execute("""
        ...
        """, (
            ...
            item.planned_yield_g, item.confidence_score, item.house_number,
...
async def get_harvest_plan(target_date: str):
    """Get the current harvest plan for a specific date"""
    try:
        # Validate date format
        date.fromisoformat(target_date)
        ...
        # Calculate target progress (4kg = 4000g per crop type)
        target_progress = {}
        for crop_type, data in crops_by_type.items():
            target_g = 4000  # 4kg target
